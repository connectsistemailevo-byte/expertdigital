...
const MiniMap: React.FC<MiniMapProps> = ({
  ...
  if (!mapboxToken) {
    return <div className={`${className} bg-gradient-to-br from-primary/20 to-primary/10 flex items-center justify-center`}>
  ...
  if (location.loading || location.latitude === 0 && location.longitude === 0) {
    return <div className={`${className} bg-gradient-to-br from-muted to-muted/50 flex items-center justify-center`}>
        <div className="text-center p-4">
  ...
  return <div className={`${className} relative min-h-[160px]`} style={{
    minHeight: '160px'
  }}>
      <div ref={mapContainer} className="absolute inset-0 w-full h-full" style={{
      minHeight: '160px'
    }} />
      
      {/* GPS Center Button */}
      <button onClick={handleCenterOnGPS} className="absolute top-2 right-2 z-10 w-8 h-8 bg-white rounded-lg shadow-lg border border-gray-200 flex items-center justify-center hover:bg-gray-50 transition-colors" title="Centralizar na localização GPS">
      ...
      <div className="absolute bottom-1 left-1 z-10 flex items-center gap-1 bg-white/90 backdrop-blur-sm px-1.5 py-0.5 rounded shadow-sm">
        ...
        <span className="text-[8px] text-secondary-foreground font-semibold">ShowTime Creative</span>
      </div>
      
      {isDragging && <div className="absolute bottom-2 left-1/2 -translate-x-1/2 bg-black/70 text-white text-xs px-3 py-1 rounded-full z-10">
